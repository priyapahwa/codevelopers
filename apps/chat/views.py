...
def room_auth(request):
    ...
    if request.method == "POST":
        ...
        if request_type == "set_pass":
            if len(room_pass) < 3:
                return JsonResponse(
                    data={"msg": "Password length must be greater than 3 characters"},
                    status=400,
                )
            if is_room_occupied(room_name):
                return JsonResponse(
                    data={"msg": "Room is already occupied!"}, status=400
                )
            ...
            if len(chatroom_qs) == 0:  # does not exist in database
                ...
                chatroom = ChatRoom(
                    room_name=room_name, room_password=room_pass
                )  # don't store in raw format TODO
                chatroom.save()
            else:  # already exists
            ...
            chatroom.save()
            return JsonResponse(
                data={"msg": "Successfully set the password for the room."}, status=201
            )
            ...
            if chatroom.room_password != room_pass:
                ...
                chat_user = ChatUser.objects.get(chat_user=request.user)
                chatroom.chat_users.add(
                    chat_user
                )  # check what happens if already exists TODO
                chatroom.save()
                return JsonResponse(
                    data={"msg": "Validated password, authorized access to room."},
                    status=200,
                )
